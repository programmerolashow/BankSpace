...
export async function GET(request: Request) {
  try {
    ...
    if (client.transaction && typeof client.transaction.findMany === "function") {
      try {
        const [trList, count, successAgg, pendC, failC] = await Promise.all([
        ...
        // Parse failure reasons from description/metadata if present
        transfers = trList.map((t: any) => {
          let failureReason = null
          if (t.status === "FAILED" || t.status === "REVERSED") {
            failureReason = t.description || t.note || "Transfer failed or was reversed by processing bank."
            if (t.metadata) {
              try {
                const meta = JSON.parse(t.metadata)
                if (meta.failureReason || meta.reason || meta.gatewayResponse) {
                  failureReason = meta.failureReason || meta.reason || meta.gatewayResponse
                }
              } catch {
                // Ignore parse error
              }
          ...
          return {
            ...t,
            failureReason,
            provider: t.providerRef ? "Paystack Direct Transfer" : "BankSpace Core Settlement",
          }
        })
        ...
        console.warn("[Admin Transfers Query Notice]:", err)
      }
    }
