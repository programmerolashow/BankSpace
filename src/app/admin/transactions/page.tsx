...
export default function AdminTransactionsPage() {
  ...
  return (
    <AdminLayout title="Transaction Management Console">
      <div className="space-y-8 pb-12">
        {/* HEADER BAR */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          <div>
            <h1 className="text-2xl font-black text-white flex items-center gap-3">
              <History className="h-7 w-7 text-indigo-400" /> Transaction Management Console
            </h1>
            <p className="text-xs text-slate-400 mt-1">
              Audit money movement transactions, Paystack provider references, fees, and status logs.
            </p>
          </div>
          <button
          ...
          >
            <RefreshCw className={`h-4 w-4 text-indigo-400 ${isLoading ? "animate-spin" : ""}`} /> Refresh Transactions
          </button>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-5 space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Transactions Count</span>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <SlidersHorizontal className="h-5 w-5 text-indigo-400" /> Filter & Search Transactions
                </h2>
                <p className="text-xs text-slate-400">Server-side database paginated transaction registry</p>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5 border-b border-slate-800 pb-4 text-xs font-semibold">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-slate-200 outline-none"
                />
              </div>
          ...
          {isLoading ? (
            <div className="py-16 text-center text-slate-400 space-y-3">
              ...
              <p className="text-xs font-semibold">Loading transaction records from database...</p>
            </div>
            ...
            <div className="py-16 text-center text-slate-500 space-y-2">
              ...
              <p className="text-xs">No records matching selected search and filters.</p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="border-b border-slate-800 text-slate-400 font-bold uppercase tracking-wider">
                    <tr>
                      <th className="pb-3">Reference / Provider Ref</th>
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-slate-800/30 transition-colors">
                          <td className="py-4">
                            ...
                            {t.providerRef && <p className="font-mono text-[10px] text-indigo-400">Ref: {t.providerRef}</p>}
                          </td>
                          <td className="py-4">
                          ...
                          <td className="py-4">
                            ...
                            <p className="text-[11px] text-slate-400">➔ {t.recipientName || t.accountNumber}</p>
                          </td>
                          ...
                          <td className="py-4 text-right">
                            ...
                            >
                              <Eye className="h-3.5 w-3.5 text-indigo-400" /> Details
                            </button>
                          </td>
                        </tr>
                      )
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-slate-800 pt-4 text-xs font-semibold text-slate-400">
                <span>
                  Showing Page <strong className="text-white">{pagination.page}</strong> of <strong className="text-white">{pagination.totalPages}</strong> (Total <strong className="text-indigo-400">{pagination.total}</strong> transactions)
                </span>
            ...
            <div className="relative w-full max-w-2xl rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl z-50 max-h-[90vh] overflow-y-auto space-y-6">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div>
                  <h3 className="text-lg font-black text-white flex items-center gap-2">
                    <History className="h-5 w-5 text-indigo-400" /> Transaction Audit Details
                  </h3>
              ...
              <div className="grid gap-4 sm:grid-cols-3 font-semibold text-xs">
                <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-1">
                  <span className="text-[10px] text-slate-500 uppercase font-bold">Transaction Amount</span>
                ...
                <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-1">
                  ...
                  <p className="font-bold text-emerald-400">{selectedTx.status}</p>
                </div>
                <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-1">
                  <span className="text-[10px] text-slate-500 uppercase font-bold">Transaction Type</span>
                  <p className="font-bold text-indigo-400">{selectedTx.type}</p>
                </div>
              </div>

              {/* Payment Provider & Reference Info */}
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-3 text-xs font-mono">
                <h4 className="text-[10px] font-sans font-bold uppercase text-slate-400">Payment Gateway & System Audit References</h4>
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  <div>
                    <span className="text-[10px] text-slate-500">Payment Provider Ref (Paystack/Flutterwave)</span>
                  ...
                  <div>
                    ...
                    <p className="text-slate-300">₦{Number(selectedTx.fee || 0).toLocaleString()}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500">Currency</span>
                    <p className="text-slate-300">{selectedTx.currency || "NGN"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-3 text-xs">
                <h4 className="text-[10px] font-bold uppercase text-slate-400">Counterparty Details</h4>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1 border-r border-slate-800/80 pr-4">
                    <span className="text-[10px] text-slate-500 font-bold uppercase">Sender Party</span>
                    <p className="font-bold text-white">{selectedTx.senderName}</p>
                    <p className="font-mono text-slate-400">Bank: {selectedTx.senderAccount?.bankName || selectedTx.bankName || "BankSpace MFB"}</p>
                    {selectedTx.senderAccount?.user && (
                      <p className="text-[11px] text-indigo-400">User: {selectedTx.senderAccount.user.email}</p>
                    )}
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-500 font-bold uppercase">Recipient Party</span>
                    <p className="font-bold text-white">{selectedTx.recipientName}</p>
                    <p className="font-mono text-slate-400">NUBAN: {selectedTx.accountNumber}</p>
                    {selectedTx.recipientAccount?.user && (
                      <p className="text-[11px] text-indigo-400">User: {selectedTx.recipientAccount.user.email}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {(selectedTx.description || selectedTx.note) && (
                <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-1 text-xs">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Note / Memo</span>
                  <p className="text-slate-300">{selectedTx.description || selectedTx.note}</p>
                </div>
              )}
            </div>
          </div>
